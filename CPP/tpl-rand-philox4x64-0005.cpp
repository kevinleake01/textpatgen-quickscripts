...
// ----- Written in ISO/IEC 14882:2026 standard C++
...
// randmax, and randseed to your preferences.
  ...
  /* Random number generator, requires C++26 or later */
  std::philox4x64 generator(randseed);
  ...
  for (num=1; num<=finish; num++)
  {
    for (size=0; size<width-1; size++)
    {
      ...
      std::fprintf(fp,"X-%02lX ", randnum);  /* Print this number */
      num++;
    }
    randnum=distribution(generator);  /* Get the random number */
    std::fprintf(fp,"X-%02lX\n", randnum);  /* Print this number */
  }
  fclose(fp);
  return 0;
}
